(window.webpackJsonp = window.webpackJsonp || []).push([
    [1], {
        "+00n": function (e, a, t) {
            e.exports = t.p + "static/47-d043279cb434d08e86cfae089b6a3175.png"
        },
        "+zAv": function (e, a, t) {
            e.exports = t.p + "static/11-2bb98f8b24a111b23ca5a8c31faa7a9e.png"
        },
        "/CrK": function (e, a, t) {
            e.exports = t.p + "static/51-0f2df59d00c512de2dc1e8e62e78873d.png"
        },
        "/PgY": function (e, a, t) {
            e.exports = t.p + "static/38-85a0ff0dc52f68f32300982e1a9a16ae.png"
        },
        "1FFf": function (e, a, t) {
            e.exports = t.p + "static/27-8e38fedad35593ef9f8b26574c8ad6f8.png"
        },
        "2NWp": function (e, a, t) {
            e.exports = t.p + "static/22-caef992802302f7d219074340d7d6597.png"
        },
        "3Ysn": function (e, a, t) {
            e.exports = t.p + "static/39-fa843f8d4934cc5ddd2d6cdc12376982.png"
        },
        "3rdH": function (e, a, t) {
            e.exports = t.p + "static/32-7b001fcf724c4fa60589e9eafa614d9e.png"
        },
        "4O48": function (e, a, t) {
            e.exports = t.p + "static/52-9068f037fe64217d03a9b128cd544bf3.png"
        },
        "5HI6": function (e, a, t) {
            e.exports = t.p + "static/18-b7cf9230f2416a232579beb9f8365e29.png"
        },
        "6ROU": function (e, a, t) {
            e.exports = t.p + "static/25-988c8e12060e6492762a902ebac9c72a.png"
        },
        "6aLc": function (e, a, t) {
            e.exports = t.p + "static/41-53020fe7bd58cae8db4f1a397f6cfb37.png"
        },
        AdGX: function (e, a, t) {
            e.exports = t.p + "static/26-353c1bd99700bedbb2acf1fae9b487e7.png"
        },
        AyFv: function (e, a, t) {
            e.exports = t.p + "static/16-ea37f3f448c17e76621b51ce7a1ce769.png"
        },
        BkY8: function (e, a, t) {
            e.exports = t.p + "static/46-45488168e4d19e864678418dbf03e521.png"
        },
        "C+Yf": function (e, a, t) {
            e.exports = t.p + "static/5-606a8ea696abe3113f42deb2d0b0bfb3.png"
        },
        EIrb: function (e, a, t) {
            e.exports = t.p + "static/tao_te_ching-4eb70e1938f08a292df9580368754294.png"
        },
        EPhf: function (e, a, t) {
            e.exports = t.p + "static/1-af802b199264af67a31ae758ec52c2b2.png"
        },
        GMs1: function (e, a, t) {
            e.exports = t.p + "static/14-4ada3e8f65fe1cc23ce08fe778878dc5.png"
        },
        GQyj: function (e, a, t) {
            e.exports = t.p + "static/12-350598a7f6931bef0eef1343256eed23.png"
        },
        GctI: function (e, a, t) {
            e.exports = t.p + "static/42-ed3f81d28971a7269d36cf50dd7e80b4.png"
        },
        Hdx9: function (e, a, t) {
            e.exports = t.p + "static/19-b13ac1b4e2645f729a75810188a3c35b.png"
        },
        IiDn: function (e, a, t) {
            e.exports = t.p + "static/54-c1a71a345733253cd5a934568973bed5.png"
        },
        Chessimage: function (e, a, t) {
            e.exports = t.p + "https://fritz.chessbase.com"
        },
        KKSa: function (e, a, t) {
            e.exports = t.p + "static/editedshelf.webp"
        },
        KKSb: function (e, a, t) {
            e.exports = t.p + "static/desk.png"
        },
        Ml1u: function (e, a, t) {
            e.exports = t.p + "static/23-dade77ddab05be89db9b9df36e3856bf.png"
        },
        OKro: function (e, a, t) {
            e.exports = t.p + "static/33-792798b4c1f1554dfd377ec129f6ffe6.png"
        },
        Odoy: function (e, a, t) {
            e.exports = t.p + "static/43-32710452e04ff30b2c7d29678180bfd2.png"
        },
        PBXW: function (e, a, t) { },
        PUo7: function (e, a, t) {
            e.exports = t.p + "static/55-11b95c0546c27aaa25bbb87d4457f587.png"
        },
        T3yo: function (e, a, t) {
            e.exports = t.p + "static/30-eb09f3a9798270590b59fbac62e0fd48.png"
        },
        UDTW: function (e, a, t) {
            e.exports = t.p + "static/49-9665413ac80ccca0046ac9c1db1a4836.png"
        },
        UXb8: function (e, a, t) {
            "use strict";
            t.r(a),
                t.d(a, "Shelf", (function () {
                    return ya
                }),);

            // t.r(a),
            //     t.d(a, "Desk", (function () {
            //         return ya2
            //     }));
            var n, o = t("q1tI"),
                s = t.n(o),
                r = (t("zjTt"),
                    t("KKSa")),
                r2 = (t("zjTt"),
                    t("KKSb")),
                i = t.n(r),
                i2 = t.n(r2),
                c = t("dCk2"),
                l = t.n(c),
                m = t("EIrb"),
                d = t.n(m),
                u = t("Zl9z"),
                h = t.n(u),
                g = t("dPJA"),
                M = t.n(g),
                L = t("sLg/"),
                E = t.n(L),
                f = (t("PBXW"),
                    t("EPhf")),
                p = t.n(f),
                b = t("Wftt"),
                v = t.n(b),
                C = t("gc8F"),
                N = t.n(C),
                y = t("pJ8i"),
                x = t.n(y),
                w = t("C+Yf"),
                k = t.n(w),
                T = t("i5yB"),
                W = t.n(T),
                j = t("arV4"),
                z = t.n(j),
                G = t("r3ca"),
                S = t.n(G),
                Y = t("rpKv"),
                D = t.n(Y),
                A = t("lWVi"),
                I = t.n(A),
                B = t("+zAv"),
                F = t.n(B),
                X = t("GQyj"),
                R = t.n(X),
                V = t("iHCP"),
                P = t.n(V),
                O = t("GMs1"),
                H = t.n(O),
                U = t("itsj"),
                q = t.n(U),
                K = t("AyFv"),
                Z = t.n(K),
                J = t("joJ0"),
                Q = t.n(J),
                _ = t("5HI6"),
                $ = t.n(_),
                ee = t("Hdx9"),
                ae = t.n(ee),
                te = t("uW2Z"),
                ne = t.n(te),
                oe = t("tQ7i"),
                se = t.n(oe),
                re = t("2NWp"),
                ie = t.n(re),
                ce = t("Ml1u"),
                le = t.n(ce),
                me = t("h4WW"),
                de = t.n(me),
                ue = t("6ROU"),
                he = t.n(ue),
                ge = t("AdGX"),
                Me = t.n(ge),
                Le = t("1FFf"),
                Ee = t.n(Le),
                fe = t("yHTZ"),
                pe = t.n(fe),
                be = t("edUf"),
                ve = t.n(be),
                Ce = t("T3yo"),
                Ne = t.n(Ce),
                ye = t("hVpp"),
                xe = t.n(ye),
                we = t("3rdH"),
                ke = t.n(we),
                Te = t("OKro"),
                We = t.n(Te),
                je = t("drBJ"),
                ze = t.n(je),
                Ge = t("kdU0"),
                Se = t.n(Ge),
                Ye = t("uznA"),
                De = t.n(Ye),
                Ae = t("hVzG"),
                Ie = t.n(Ae),
                Be = t("/PgY"),
                Fe = t.n(Be),
                Xe = t("3Ysn"),
                Re = t.n(Xe),
                Ve = t("mPLV"),
                Pe = t.n(Ve),
                Oe = t("6aLc"),
                He = t.n(Oe),
                Ue = t("GctI"),
                qe = t.n(Ue),
                Ke = t("Odoy"),
                Ze = t.n(Ke),
                Je = t("t5jt"),
                Qe = t.n(Je),
                _e = t("lyYz"),
                $e = t.n(_e),
                ea = t("BkY8"),
                aa = t.n(ea),
                ta = t("+00n"),
                na = t.n(ta),
                oa = t("nlLN"),
                sa = t.n(oa),
                ra = t("UDTW"),
                ia = t.n(ra),
                ca = t("dpWB"),
                la = t.n(ca),
                ma = t("/CrK"),
                da = t.n(ma),
                ua = t("4O48"),
                ha = t.n(ua),
                ga = t("aIvw"),
                Ma = t.n(ga),
                La = t("IiDn"),
                Ea = t.n(La),
                Chess = t("Chessimage"),
                Chessim = t.n(Chess),
                fa = t("PUo7"),
                pa = t.n(fa),
                ba = t("V/pM"),
                va = t.n(ba),
                Ca = ((n = {})[p.a] = "The Cold Start Problem, Andrew Chen",
                    n[v.a] = "How to Do Nothing, Jenny Odell",
                    n[N.a] = "A Year with Swollen Appendices, Brian Eno",
                    n[x.a] = "The Empathy Diaries, Sherry Turkle",
                    n[k.a] = "Maus, Art Spiegelman",
                    n[W.a] = "Pilgrim at Tinker Creek, Annie Dillard",
                    n[z.a] = "Notes on the Synthesis of Form, Christopher Alexander",
                    n[S.a] = "A Pattern Language, Christopher Alexander",
                    n[D.a] = "Prisoners' Inventions, Angelo",
                    n[I.a] = "The New Games Book, Andrew Fluegelman",
                    n[F.a] = "A *New* Program for Graphic Design, David Reinfurt",
                    n[R.a] = "Impro, Keith Johnstone",
                    n[P.a] = "Turtles, Termites, and Traffic Jams, Mitchel Resnick",
                    n[H.a] = "Lifelong Kindergarten, Mitchel Resnick",
                    n[q.a] = "Vehicles, Valentino Braitenberg",
                    n[Z.a] = "Notes on my Dunce Cap, Jesse Ball",
                    n[Q.a] = "Pedagogy of the Oppressed, Paulo Freire",
                    n[$.a] = "Conflict is Not Abuse, Sarah Schulman",
                    n[ae.a] = "After Accountablity, Pinko Collective",
                    n[ne.a] = "The Case for Black Reparations, Boris Bittker",
                    n[se.a] = "How Buildings Learn, Stewart Brand",
                    n[ie.a] = "Landscape and Desire, John Bell",
                    n[le.a] = "68 Ways to make Really Big Puppets, Sara Peattie and the Puppeteers Collective",
                    n[de.a] = "Making Comics, Lynda Barry",
                    n[he.a] = "Children's Experiences in Art, Pearl Greenberg",
                    n[Me.a] = "Taking a Line for a Walk Assignments in Design Education, Nina Paim",
                    n[Ee.a] = "Understanding Comics, Scott McCloud",
                    n[pe.a] = "The Phenomenon of Life, Christopher Alexander",
                    n[ve.a] = "Ecology of The Planted Aquarium, Diana Walstad",
                    n[Ne.a] = "The Soul of a New Machine, Tracy Kidder",
                    n[xe.a] = "Gödel, Escher, Bach, Douglas Hofstadter",
                    n[ke.a] = "Invisible Cities, Italo Calvino",
                    n[We.a] = "Chelsea Girls, Eileen Myles",
                    n[ze.a] = "Paul Takes The Form of a Mortal Girl, Andrea Lawlor",
                    n[Se.a] = "Darryl, Jackie Ess",
                    n[De.a] = "Fantasian, Larissa Pham",
                    n[Ie.a] = "The Dispossessed, Ursula K. Le Guin",
                    n[Fe.a] = "Idoru, William Gibson",
                    n[Re.a] = "The Overstory, Richard Powers",
                    n[Pe.a] = "The Living, Annie Dillard",
                    n[He.a] = "Wolf in White Van, John Darnielle",
                    n[qe.a] = "Seeing is forgetting the name of the thing one sees, Lawrence Weschler",
                    n[Ze.a] = "Underground, Haruki Murakami",
                    n[Qe.a] = "Lonely Hearts Killer, Tomoyuki Hoshino",
                    n[$e.a] = "There is no antimimetics Division, qntm",
                    n[aa.a] = "The Plauge, Albert Camus",
                    n[na.a] = "The Castle, Franz Kafka",
                    n[sa.a] = "Soft Corruptor, Everest Pipkin",
                    n[ia.a] = "Ficciones, Jorge Luis Borges",
                    n[la.a] = "Feeling Great About My Butt, Kawandeep Virdee",
                    n[da.a] = "Waiting for Godot, Samuel Beckett",
                    n[ha.a] = "Parable of the Sower, Octavia Butler",
                    n[Ma.a] = "Well Played Game, Bernard De Koven",
                    n[Ea.a] = "The Earthsea Trilogy, Ursula K. Le Guin",
                    n[Chessim.a] = "Chessboard",
                    n[pa.a] = "Moby Dick, Herman Melville",
                    n[va.a] = "Dream Work, Mary Oliver",
                    n),
                Na = function (e) {
                    return s.a.createElement("svg", Object.assign({
                        xmlns: "http://www.w3.org/2000/svg",
                        fill: "none",
                        viewBox: "0 0 4033 3025"
                    }, e), s.a.createElement("path", {
                        d: "M3083.34 194.976C3084 193.273 3086.41 193.273 3087.07 194.976L3095.59 216.961C3095.88 217.693 3096.56 218.192 3097.35 218.235L3120.89 219.546C3122.71 219.647 3123.46 221.939 3122.04 223.093L3103.77 237.99C3103.16 238.486 3102.89 239.293 3103.1 240.052L3109.12 262.847C3109.59 264.612 3107.64 266.029 3106.11 265.039L3086.29 252.261C3085.63 251.836 3084.78 251.836 3084.12 252.261L3064.31 265.039C3062.77 266.029 3060.82 264.612 3061.29 262.847L3067.32 240.052C3067.52 239.293 3067.26 238.486 3066.65 237.99L3048.37 223.093C3046.96 221.939 3047.7 219.647 3049.53 219.546L3073.07 218.235C3073.85 218.192 3074.54 217.693 3074.82 216.961L3083.34 194.976Z",
                        fill: "#E7CC86"
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 75.6635,
                        y: 19.7271,
                        width: 413.627,
                        height: 1342.38,
                        id: Ea.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 530.634,
                        y: 141.33,
                        width: 134.249,
                        height: 1219.05,
                        transform: "rotate(1.5808 530.634 141.33)",
                        id: ha.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 86.3391,
                        y: 1604.35,
                        width: 164.893,
                        height: 1328.09,
                        transform: "rotate(-0.0767265 86.3391 1604.35)",
                        id: p.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 255.336,
                        y: 1693.06,
                        width: 144.224,
                        height: 1250.73,
                        transform: "rotate(-0.0767265 255.336 1693.06)",
                        id: v.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 406.807,
                        y: 1710.67,
                        width: 235.217,
                        height: 1233.44,
                        transform: "rotate(-0.0767265 406.807 1710.67)",
                        id: N.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 655.67,
                        y: 1632.37,
                        width: 61.1048,
                        height: 1300.07,
                        transform: "rotate(-0.0767265 655.67 1632.37)",
                        id: x.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 729.694,
                        y: 1756.2,
                        width: 95.9747,
                        height: 1186.96,
                        transform: "rotate(-0.0767265 729.694 1756.2)",
                        id: k.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 862.009,
                        y: 1794,
                        width: 77.9461,
                        height: 1149.79,
                        transform: "rotate(-0.0767265 862.009 1794)",
                        id: W.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 956.46,
                        y: 1773.91,
                        width: 73.3656,
                        height: 1172.85,
                        transform: "rotate(-0.0767265 956.46 1773.91)",
                        id: z.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1050.74,
                        y: 1787.9,
                        width: 258.598,
                        height: 1159.01,
                        transform: "rotate(-0.0767265 1050.74 1787.9)",
                        id: S.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1340.44,
                        y: 1723.05,
                        width: 63.4846,
                        height: 1223.92,
                        transform: "rotate(-0.0767265 1340.44 1723.05)",
                        id: D.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1423.06,
                        y: 1624.83,
                        width: 90.4812,
                        height: 1335.43,
                        transform: "rotate(-0.0767265 1423.06 1624.83)",
                        id: Ma.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1528.64,
                        y: 1647.99,
                        width: 68.2081,
                        height: 1312.36,
                        transform: "rotate(-0.0767265 1528.64 1647.99)",
                        id: I.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1616.15,
                        y: 1660.44,
                        width: 90.4812,
                        height: 1300.07,
                        transform: "rotate(-0.0767265 1616.15 1660.44)",
                        id: F.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1722.61,
                        y: 1767.56,
                        width: 68.4639,
                        height: 1209.68,
                        transform: "rotate(-0.0767265 1722.61 1767.56)",
                        id: R.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1805.65,
                        y: 1665.96,
                        width: 63.7495,
                        height: 1294.13,
                        transform: "rotate(-0.0767265 1805.65 1665.96)",
                        id: P.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1870.16,
                        y: 1797.16,
                        width: 69.9976,
                        height: 1163.41,
                        transform: "rotate(-0.0767265 1870.16 1797.16)",
                        id: H.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1949.98,
                        y: 1807.73,
                        width: 56.2628,
                        height: 1176.99,
                        transform: "rotate(-0.0767265 1949.98 1807.73)",
                        id: q.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2018.29,
                        y: 1907.02,
                        width: 42.6417,
                        height: 1095.31,
                        transform: "rotate(-0.0767265 2018.29 1907.02)",
                        id: Z.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2130.97,
                        y: 1659.71,
                        width: 56.2199,
                        height: 1342.03,
                        transform: "rotate(2.31762 2130.97 1659.71)",
                        id: Q.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2208.53,
                        y: 1660.68,
                        width: 104.525,
                        height: 1330.8,
                        transform: "rotate(2.79298 2208.53 1660.68)",
                        id: $.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1.84644,
                        y: 2.1489,
                        width: 84.9253,
                        height: 1195.85,
                        transform: "matrix(0.997018 0.0771752 -0.0737988 0.997273 2329.77 1794.89)",
                        id: ae.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1.8715,
                        y: 2.12711,
                        width: 76.0053,
                        height: 1256.88,
                        transform: "matrix(0.997854 0.0654834 -0.0621042 0.99807 2402.43 1734.66)",
                        id: ne.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.95948,
                        y: 2.04634,
                        width: 90.6364,
                        height: 1520.3,
                        transform: "matrix(0.0199857 0.9998 -0.999727 0.0233709 3934.63 2808.78)",
                        id: ve.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.97288,
                        y: 2.03343,
                        width: 100.587,
                        height: 1494.47,
                        transform: "matrix(0.0134197 0.99991 -0.999859 0.0168053 3934.83 2694.16)",
                        id: pe.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.98683,
                        y: 2.0198,
                        width: 74.7629,
                        height: 1437.78,
                        transform: "matrix(0.00653403 0.999979 -0.999951 0.00991996 3870.51 2591.09)",
                        id: Ee.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.98624,
                        y: 2.02038,
                        width: 114.211,
                        height: 1398.44,
                        transform: "matrix(0.00682641 0.999977 -0.999948 0.0102123 3838.89 2463.9)",
                        id: Me.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.95948,
                        y: 2.04634,
                        width: 86.7926,
                        height: 1435.08,
                        transform: "matrix(0.0199857 0.9998 -0.999727 0.0233709 3885.59 2338.5)",
                        id: he.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.95948,
                        y: 2.04634,
                        width: 79.1448,
                        height: 1324.79,
                        transform: "matrix(0.0199857 0.9998 -0.999727 0.0233709 3780 2243.63)",
                        id: de.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.95948,
                        y: 2.04634,
                        width: 79.0247,
                        height: 1148.17,
                        transform: "matrix(0.0199857 0.9998 -0.999727 0.0233709 3610.39 2106)",
                        id: se.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.95948,
                        y: 2.04634,
                        width: 20.5087,
                        height: 1439.4,
                        transform: "matrix(0.0199857 0.9998 -0.999727 0.0233709 3902.05 2214.51)",
                        id: le.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: -1.95948,
                        y: 2.04634,
                        width: 20.5087,
                        height: 1425.22,
                        transform: "matrix(0.0199857 0.9998 -0.999727 0.0233709 3889.87 2185.73)",
                        id: ie.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 702.571,
                        y: 137.516,
                        width: 171.19,
                        height: 1219.05,
                        transform: "rotate(1.5808 702.571 137.516)",
                        id: Ne.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 911.434,
                        y: 14.9485,
                        width: 149.734,
                        height: 1329.76,
                        transform: "rotate(1.5808 911.434 14.9485)",
                        id: xe.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1065.17,
                        y: 204.38,
                        width: 66.6203,
                        height: 1159.3,
                        id: ke.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1144.43,
                        y: 204.38,
                        width: 93.55,
                        height: 1159.3,
                        id: We.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1250.63,
                        y: 204.38,
                        width: 88.4842,
                        height: 1159.3,
                        id: ze.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1365.47,
                        y: 141.275,
                        width: 74.3873,
                        height: 1220.83,
                        id: Se.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1466.22,
                        y: 359.438,
                        width: 74.3873,
                        height: 1004.24,
                        id: De.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1566.96,
                        y: 412.421,
                        width: 122.224,
                        height: 951.256,
                        id: Ie.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1715.55,
                        y: 63.3594,
                        width: 122.224,
                        height: 1296.5,
                        id: Fe.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 1841.77,
                        y: 204.38,
                        width: 172.52,
                        height: 1159.3,
                        id: Re.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2021.95,
                        y: 226.196,
                        width: 131.614,
                        height: 1137.48,
                        id: Pe.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2161.22,
                        y: 160.747,
                        width: 131.614,
                        height: 1187.64,
                        id: pa.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2319.2,
                        y: 176.035,
                        width: 86.8138,
                        height: 1187.64,
                        id: He.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2421.08,
                        y: 87.2721,
                        width: 73.9155,
                        height: 1268.23,
                        transform: "rotate(1 2421.08 87.2721)",
                        id: qe.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 3200.78,
                        y: 97.2472,
                        width: 69.3272,
                        height: 697.799,
                        transform: "rotate(87.5004 3200.78 97.2472)",
                        id: sa.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2508.26,
                        y: 204.414,
                        width: 101.229,
                        height: 1157.64,
                        transform: "rotate(1 2508.26 204.414)",
                        id: Ze.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2610.65,
                        y: 224.626,
                        width: 87.9525,
                        height: 1148.33,
                        id: Qe.a,
                        transform: "rotate(1 2610.65 224.626)"
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2702.59,
                        y: 361.366,
                        width: 87.9525,
                        height: 1009.74,
                        transform: "rotate(1 2702.59 361.366)",
                        id: $e.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2794.53,
                        y: 345.983,
                        width: 94.2227,
                        height: 1025.41,
                        transform: "rotate(1 2794.53 345.983)",
                        id: aa.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 2913.09,
                        y: 208.828,
                        width: 127.167,
                        height: 1152.83,
                        transform: "rotate(1 2913.09 208.828)",
                        id: na.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 3061.19,
                        y: 211.117,
                        width: 57.3448,
                        height: 1163.45,
                        transform: "rotate(1 3061.19 211.117)",
                        id: ia.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 3130.36,
                        y: 219.173,
                        width: 64.9578,
                        height: 1149.48,
                        transform: "rotate(1 3130.36 219.173)",
                        id: da.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 3196.37,
                        y: 170.454,
                        width: 34.8338,
                        height: 1197.5,
                        transform: "rotate(1 3196.37 170.454)",
                        id: va.a
                    }), s.a.createElement("rect", {
                        onMouseEnter: e.onMouseEnter,
                        onMouseLeave: e.onMouseLeave,
                        x: 3235.23,
                        y: 49.8379,
                        width: 71.2036,
                        height: 1318.77,
                        id: la.a
                    }))
                },
                Na2 = function (e) {
                    return s.a.createElement("svg", Object.assign({
                        xmlns: "http://www.w3.org/2000/svg",
                        fill: "none",
                        viewBox: "0 0 3809 1327"
                    }, e),

                        // s.a.createElement("div", {
                        //     className: "chessdiv",
                        //     style: L
                        // },
                        //     o.a.createElement("iframe", {
                        //         id: "chessIframe",
                        //         style: {
                        //             width: "100%",        // Full width of the container div
                        //             height: "100%",       // Full height of the container div     // Remove border      
                        //             transition: "opacity 0.3s ease" // Smooth transition
                        //         },
                        //         src: "https://fritz.chessbase.com",
                        //     })
                        // ),
                        // s.a.createElement("rect", {
                        //     onMouseEnter: e.onMouseEnter,
                        //     onMouseLeave: e.onMouseLeave,
                        //     x: -1400,
                        //     y: 1200,
                        //     width: 2000,
                        //     height: 1342.38,
                        //     id: Ea.a
                        // })
                        s.a.createElement("rect", {
                            // onMouseEnter: e.onMouseEnter,
                            // onMouseLeave: e.onMouseLeave,
                            onMouseEnter: function (e) {
                                document.getElementById("chessIframe").style.opacity = 1; // Show iframe on hover
                            },
                            onMouseLeave: function (e) {
                                document.getElementById("chessIframe").style.opacity = 0; // Hide iframe when not hovering
                            },
                            x: 300,
                            y: 450,
                            width: 100,
                            height: 600,
                            id: "chessrect"
                            
                            //     //id: "https://fritz.chessbase.com"
                            
                        })
                    )
                };
                
            function ya() {
                var e, a, t, n, o, r = s.a.useState(!1),
                    c = r[0],
                    l = r[1],
                    m = s.a.useRef(null),
                    d = null === (e = m.current) || void 0 === e ? void 0 : e.getBoundingClientRect(),
                    u = ((null == d ? void 0 : d.left) + (null == d ? void 0 : d.right)) / 2,
                    h = ((null == d ? void 0 : d.top) + (null == d ? void 0 : d.bottom)) / 2,
                    g = "TranslateX(0%)",
                    M = "TranslateY(-50%)",
                    L = {};
                if ((null == c || null === (a = c.pos) || void 0 === a ? void 0 : a.bottom) > h) {
                    var E, f, p = ((null == c || null === (E = c.pos) || void 0 === E ? void 0 : E.top) + (null == c || null === (f = c.pos) || void 0 === f ? void 0 : f.bottom)) / 2;
                    L.bottom = (null == d ? void 0 : d.bottom) - p, M = "TranslateY(50%)"
                } else {
                    var b, v;
                    L.top = (null == c || null === (b = c.pos) || void 0 === b ? void 0 : b.top) + (null == c || null === (v = c.pos) || void 0 === v ? void 0 : v.height) / 2 - (null == d ? void 0 : d.top)
                } (null == c || null === (t = c.pos) || void 0 === t ? void 0 : t.right) < u ? L.left = (null == c || null === (n = c.pos) || void 0 === n ? void 0 : n.right) - (null == d ? void 0 : d.left) : (L.left = (null == c || null === (o = c.pos) || void 0 === o ? void 0 : o.left) - (null == d ? void 0 : d.left), g = "TranslateX(-100%)");
                return L.transform = g + " " + M, s.a.createElement("div", {
                    className: "main-shelf",
                    ref: m
                }, s.a.createElement("img", {
                    src: i.a
                }), s.a.createElement("img", {
                    id: "overlay-img",
                    src: null == c ? void 0 : c.url,
                    style: L
                }), s.a.createElement(Na, {
                    onMouseEnter: function (e) {
                        e.target.id != (null == c ? void 0 : c.url) && l({
                            url: e.target.id,
                            pos: e.target.getBoundingClientRect()
                        })
                    },
                    onMouseEnterChess: function (e) {
                        let chessboard = document.getElementsByClassName("chessboard");
                        let chessboard2 = document.getElementsByClassName("chessIframe");
                        chessboard.style.opacity = '1';
                        chessboard2.style.opacity = '1';
                    },
                    onMouseLeave: function (e) {
                        var a = e.target.id;
                        l((function (e) {
                            return (null == e ? void 0 : e.url) == a ? {
                                url: null
                            } : e
                        })), e.target.style.fill = "none"
                    },
                    onMouseLeaveChess: function (e) {
                        let chessboard = document.getElementsByClassName("chessboard");
                        let chessboard2 = document.getElementsByClassName("chessIframe");
                        chessboard.style.opacity = '1';
                        chessboard2.style.opacity = '1';
                    }
                }))
            }

            function ya2() {
                var e, a, t, n, o, r = s.a.useState(!1),
                    c = r[0],
                    l = r[1],
                    m = s.a.useRef(null),
                    d = null === (e = m.current) || void 0 === e ? void 0 : e.getBoundingClientRect(),
                    u = ((null == d ? void 0 : d.left) + (null == d ? void 0 : d.right)) / 2,
                    h = ((null == d ? void 0 : d.top) + (null == d ? void 0 : d.bottom)) / 2,
                    g = "TranslateX(0%)"//"TranslateX(0%)",
                M = "TranslateY(-50%)"//"TranslateY(0%)",
                L = {};
                if ((null == c || null === (a = c.pos) || void 0 === a ? void 0 : a.bottom) > h) {
                    var E, f, p = ((null == c || null === (E = c.pos) || void 0 === E ? void 0 : E.top) + (null == c || null === (f = c.pos) || void 0 === f ? void 0 : f.bottom)) / 2;
                    L.bottom =
                        (null == d ? void 0 : d.bottom) - p
                        , M = "TranslateY(50%)" //"TranslateY(0%)"

                } else {
                    var b, v;
                    L.top = (null == c || null === (b = c.pos) || void 0 === b ? void 0 : b.top) + (null == c || null === (v = c.pos) || void 0 === v ? void 0 : v.height) / 2 - (null == d ? void 0 : d.top)
                } (null == c || null === (t = c.pos) || void 0 === t ? void 0 : t.right) < u ? L.left = (null == c || null === (n = c.pos) || void 0 === n ? void 0 : n.right) - (null == d ? void 0 : d.left) : (L.left = (null == c || null === (o = c.pos) || void 0 === o ? void 0 : o.left) - (null == d ? void 0 : d.left), g = "TranslateX(-100%)"//"TranslateX(-100%)"

                )
                L.left = L.left - (.9 * L.left)
                L.bottom = L.bottom
                L.width = L.width * .25
                L.height = L.height * .25
                    ;
                s.a.useEffect(() => {
                    let slideIndex = 0;

                    function showSlides() {
                        let i;
                        let slides = document.getElementsByClassName("mySlides");

                        for (i = 0; i < slides.length; i++) {
                            slides[i].style.display = "none";
                        }

                        slideIndex++;
                        if (slideIndex > slides.length) { slideIndex = 1 }


                        slides[slideIndex - 1].style.display = "block";

                        setTimeout(showSlides, 2000); // Change image every 2 seconds
                    }
                    function chess() {
                        window.addEventListener("message", e => {
                            e['data'] && "12085795" === e['data']['id'] && document.getElementById(`${e['data']['id']}`) &&
                                (document.getElementById(`${e['data']['id']}`).style.height = `${e['data']['frameHeight'] + 30}px`);
                        });
                    }

                    // Start the slideshow
                    showSlides();
                    chess();
                }, []); // Empty dependency array ensures this runs once on mount
                return L.transform = g + " " + M, s.a.createElement("div", {
                    className: "main-shelf",
                    ref: m,
                    position: "relative"
                }, s.a.createElement("div", {
                    className: "slideshow-container"
                },
                    s.a.createElement("div", {
                        className: "mySlides fade"
                    },
                        s.a.createElement("img", {
                            src: "IMG_2405.JPG",
                            style: { width: "100%", height: "100%" }
                        })),

                    s.a.createElement("div", {
                        className: "mySlides fade"
                    },
                        s.a.createElement("img", {
                            src: "IMG_2482.JPG",
                            style: { width: "100%", height: "100%" }
                        }))
                ),

                    //      ,o.a.createElement("iframe", {
                    //         id: "chessIframe",
                    //         style: {
                    //             width: "100%",        // Full width of the container div
                    //             height: "100%",       // Full height of the container div     // Remove border      
                    //             transition: "opacity 0.3s ease" // Smooth transition
                    //         },
                    //         src: "https://fritz.chessbase.com",
                    //     })
                    // ),



                    s.a.createElement("img", {
                        src: i2.a
                    }),
                    // s.a.createElement("img", {
                    //     id: "chess-img",
                    //     src: null == c ? void 0 : c.url,
                    //     style: L
                    // })

                    s.a.createElement(Na2, {

                        onMouseEnter:
                            function (e) {
                                e.target.id != (null == c ? void 0 : c.url) && l({
                                    url: e.target.id,
                                    pos: e.target.getBoundingClientRect()
                                })
                            },
                        onMouseLeave:
                            function (e) {
                                var a = e.target.id;
                                l((function (e) {
                                    return (null == e ? void 0 : e.url) == a ? {
                                        url: null
                                    } : e
                                })), e.target.style.fill = "none"
                            }
                    }));
            }

            a.default = function (e) {
                return s.a.createElement("div", {
                    className: "shelf"
                }, s.a.createElement(ya, null, {
                    style: {
                        padding: "0px",
                        width: "100%"
                    }
                }
                ), s.a.createElement("span", {
                    style: {
                        padding: "0px",
                        width: "100%"
                    }
                }, s.a.createElement("a", {
                    href: "https://www.goodreads.com/user/show/149251509-matthew-sadowski"
                }, " ", "Goodreads ")

                ), s.a.createElement(ya2, null)
                )

            }
        },
        "V/pM": function (e, a, t) {
            e.exports = t.p + "static/56-780e28d667973f5089a79a3c8dd77a62.png"
        },
        Wftt: function (e, a, t) {
            e.exports = t.p + "static/2-a2c5c52a969b94ddf4b7a7ebd3ce35da.png"
        },
        Zl9z: function (e, a, t) {
            e.exports = t.p + "static/weaving-71caec913fb9c36bee9e9c294f9d2eeb.webp"
        },
        aIvw: function (e, a, t) {
            e.exports = t.p + "static/53-211a334caf208afd198829cad5d79bd1.png"
        },
        arV4: function (e, a, t) {
            e.exports = t.p + "static/7-b94f03f3f232d2b5580f040cc5b58e78.png"
        },
        dCk2: function (e, a, t) {
            e.exports = t.p + "static/connie-d7265812869732497199814d245d58d7.jpeg"
        },
        dPJA: function (e, a, t) {
            e.exports = t.p + "static/water-d5f5b6956c48f8a4e234c4c19ca14f0e.jpg"
        },
        dpWB: function (e, a, t) {
            e.exports = t.p + "static/50-f4e82b7b1a6a76d738436c202d2e681c.png"
        },
        drBJ: function (e, a, t) {
            e.exports = t.p + "static/34-b5ad24b9068b4f1eec70218009090211.png"
        },
        edUf: function (e, a, t) {
            e.exports = t.p + "shelteringskycover.png"
        },
        gc8F: function (e, a, t) {
            e.exports = t.p + "static/3-60783eee4d92567dc2d902211c1f61ff.png"
        },
        h4WW: function (e, a, t) {
            e.exports = t.p + "static/24-16f668b3ea4ca71878efcefcc15904a0.png"
        },
        hVpp: function (e, a, t) {
            e.exports = t.p + "static/31-8ac9dca04a41a90cb474b1172cc3435e.png"
        },
        hVzG: function (e, a, t) {
            e.exports = t.p + "static/37-94bb102f66e0871a2b8f2b7143434d65.png"
        },
        i5yB: function (e, a, t) {
            e.exports = t.p + "static/6-2de1f685af26b40a61397b87a717bc7e.png"
        },
        iHCP: function (e, a, t) {
            e.exports = t.p + "static/13-f367e77419070e31fda4edc850305c3a.png"
        },
        itsj: function (e, a, t) {
            e.exports = t.p + "static/15-10966ef1993e034abe982079713489f8.png"
        },
        joJ0: function (e, a, t) {
            e.exports = t.p + "static/17-d185344a02df4fe0873dd66c6ea748e9.png"
        },
        kdU0: function (e, a, t) {
            e.exports = t.p + "static/35-7c2101828bcccd7d5c0cb318c176d86d.png"
        },
        lWVi: function (e, a, t) {
            e.exports = t.p + "static/10-0934976cb280725469a4c093c74bfe90.png"
        },
        lyYz: function (e, a, t) {
            e.exports = t.p + "static/45-0f6bbb82bec7aa1fa5dfd7529c83e16c.png"
        },
        mPLV: function (e, a, t) {
            e.exports = t.p + "static/40-51797952a0c9f68455e6431852dd8d07.png"
        },
        nlLN: function (e, a, t) {
            e.exports = t.p + "static/48-74ad240dece1668550d26f52335b2d7c.png"
        },
        pJ8i: function (e, a, t) {
            e.exports = t.p + "static/4-8166ad59941d52b17a01299e2dd35b06.png"
        },
        r3ca: function (e, a, t) {
            e.exports = t.p + "static/8-0e97a00ef655d46857811a2b7a24d5dc.png"
        },
        rpKv: function (e, a, t) {
            e.exports = t.p + "static/9-28a5f32cf553f2d1e69da2975737f1c2.png"
        },
        "sLg/": function (e, a, t) {
            e.exports = t.p + "static/mindstorms-6ebab0b6e298148f5ab964dd2dc867f2.jpg"
        },
        t5jt: function (e, a, t) {
            e.exports = t.p + "static/44-d207d6e18854d1b6a19685ca3ee5055d.png"
        },
        tQ7i: function (e, a, t) {
            e.exports = t.p + "static/21-b0450a68d17ec67a8dca62aa8b3c5051.png"
        },
        uW2Z: function (e, a, t) {
            e.exports = t.p + "static/20-cf2a78535929aad91e57124d7b350902.png"
        },
        uznA: function (e, a, t) {
            e.exports = t.p + "static/36-86fd4de8d033fbf1687c17358fc236df.png"
        },
        yHTZ: function (e, a, t) {
            e.exports = t.p + "static/28-b72382c1ab9e7a0882a5a515d593ce8d.png"
        },
        zjTt: function (e, a, t) { }
    }
]);
//# sourceMappingURL=component---src-pages-bookshelf-js-70132e3a936d2e5014f5.js.map